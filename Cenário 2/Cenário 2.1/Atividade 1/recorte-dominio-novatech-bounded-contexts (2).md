...
## Linguagem Ubíqua — Glossário do Domínio NovaTech

Os termos abaixo devem ser usados de forma consistente por todos os membros do time e por todos os agentes de IA que operam sobre este domínio. Cada termo foi extraído da documentação formal (Anexo A) ou da Spec v2.0. A coluna "Por que precisa de definição" explicita o risco de ambiguidade para agentes de IA.

### Termos de Negócio — Logística e Atendimento

| Termo | Definição operacional | Fonte | Por que precisa de definição |
|---|---|---|---|
| **Carga perigosa** | Mercadoria classificada nas classes 1 a 6 conforme regulamentação ANTT. Exige procedimento específico de frete. | PROC-042 v2 | LLM pode confundir com "carga frágil", "carga pesada" ou interpretar como risco genérico. |
| **Frete especial** | Frete para cargas acima de 500 kg ou com dimensões fora do padrão. Sujeito a multiplicadores regionais. | PROC-042 v1 / v2 | Sem threshold explícito, "especial" é ambíguo — pode ser interpretado como frete expresso ou prioritário. |
| **Frete padrão** | Frete para cargas até 500 kg dentro das dimensões regulares, sem exigência de procedimento especial. | PROC-042 v2 | Termo de contraste com "frete especial"; sem definição, o agente não sabe o que é "não-especial". |
| **Multiplicador regional** | Fator aplicado ao valor do frete conforme a região de destino (ex.: Norte = 1.8×, Sudeste = 1.0×). | PROC-042 v2 (tabela de multiplicadores) | Os valores mudam entre versões de documento. Sem versionamento explícito, o agente pode usar valor obsoleto. |
| **Tier de cliente** | Classificação do cliente por nível de serviço: Bronze, Prata ou Ouro. Define SLAs e multiplicadores aplicáveis. | SLA-2024, POL-CLI-002 | "Gold" pode ser interpretado como metal, não como tier. "Platinum" não existe — agente deve detectar como entidade inexistente. |
| **SLA de atendimento** | Prazo máximo de resposta ao cliente conforme tier e tipo de demanda, definido em horas úteis. | SLA-2024 | Agente pode confundir SLA de atendimento (ao cliente) com SLA de triagem (interno de qualidade). |
| **Prazo de devolução** | Prazo em dias corridos para o cliente solicitar devolução, contado a partir do recebimento. Varia por tipo de produto. | POL-001 (Política de Devolução) | Há contradição conhecida entre POL-001 e PROC-DEV-012 (7 dias vs. 30 dias) — o agente deve declarar, não resolver. |

### Termos de Produto — Comportamento do Assistente

| Termo | Definição operacional | Fonte | Por que precisa de definição |
|---|---|---|---|
| **Documento formal** | Documento com código, versão, data de vigência, área proprietária e aprovação registrada. Tipos: POL, PROC, SLA, comunicado formal. | Spec v2.0 §2.1 | Distingue de FAQ informal, e-mails ou conhecimento tácito — que não são fonte válida para respostas. |
| **Documento vigente** | Documento formal com status "vigente" no sistema de gestão documental, com aprovação ativa. | Spec v2.0 §5.3 | Agente pode tratar rascunho ou documento obsoleto como fonte válida se não distinguir status. |
| **Em transição** | Documento sendo substituído por versão nova, mas ainda válido com escopo temporal ou funcional explícito. As duas versões coexistem. | Spec v2.0 §2.1 | Agente pode tratar como obsoleto e ignorar, ou como vigente sem ressalva. Precisa declarar a condição de transição. |
| **Documento obsoleto** | Documento revogado ou substituído, movido para arquivo histórico. Não elegível para recuperação pelo assistente. | Spec v2.0 §5.2 | Se o agente acessa o arquivo histórico sem restrição, pode citar fonte revogada. |
| **Lacuna documental** | Tema sobre o qual não existe cobertura na base de documentos formais vigentes. | Spec v2.0 §4 | Diferente de "caso limítrofe" (cobertura parcial ou ambígua). Agente deve declarar explicitamente, não improvisar. |
| **Caso limítrofe** | Consulta parcialmente coberta pela base, onde a informação disponível não é suficiente para uma resposta completa com alta confiança. | Spec v2.0 §4.2 | Sem definição, o agente trata como lacuna total (perde informação útil) ou como resposta completa (inventa o que falta). |
| **Contradição documental** | Dois ou mais documentos vigentes divergem sobre o mesmo tema. É um estado legítimo da base, não um bug. | Spec v2.0 §3, ADR-0004 | Agente pode tentar "resolver" escolhendo a versão mais recente — o que viola G4 (neutralidade). |
| **Nível de confiança** | Classificação obrigatória da resposta: Alto (cobertura completa, fonte única e clara), Médio (múltiplas fontes parciais ou condição de transição), Baixo (cobertura mínima ou ambígua). | Spec v2.0 §6.3, ADR-0003 | "Confiança" é vago sem critérios objetivos. Sem escala fixa, o agente calibra subjetivamente. |
| **Não aplicável** | Quarto estado de confiança, usado quando não há base para classificação (lacuna total ou entidade inexistente). | Mockup v1.1, sessão Tech Lead | Estado ausente na Spec v2.0 original — formalizado após revisão. Sem ele, o agente atribui "Baixo" a cenários onde não há resposta. |

### Termos de Governança e Processo

| Termo | Definição operacional | Fonte | Por que precisa de definição |
|---|---|---|---|
| **Pré-indexação** | Processo obrigatório de 4 etapas antes de incluir documento na base: validação formal, verificação de conflito, marcação de escopo, arquivamento da versão anterior. | Spec v2.0 §5.2 | Sem esse processo, documentos podem entrar na base sem revisão, gerando contradições silenciosas. |
| **SLA de triagem** | Prazo de 2 dias úteis para a Área de Qualidade triar conflitos detectados pelo assistente. Escalada automática ao Gerente de Qualidade se excedido. | Spec v2.0 §3.3 | Distinto do SLA de atendimento ao cliente. Confusão entre os dois gera expectativas incorretas. |
| **Ciclo de revisão** | Revisão periódica da base de conhecimento a cada 90 dias. | Spec v2.0 §5 | Agente não precisa executar, mas deve saber que a base tem cadência de atualização definida. |
| **Chamado** | Unidade de rastreabilidade do atendimento. Toda consulta ao assistente está vinculada a um chamado com ID único. | Jornada Operacional §1.1 | Sem vínculo ao chamado, a rastreabilidade (C3) e o registro de trechos (VC-08) não funcionam. |
| **Feedback estruturado** | Registro de problema com campos obrigatórios: tipo do problema, pergunta original, resposta recebida, resposta esperada, ID do chamado, documento de referência. | Jornada Operacional §3 | Se o formato for livre, a triagem não consegue priorizar por frequência e criticidade. |

### Regras de Uso do Glossário

1. **Para humanos:** usar os termos acima em toda comunicação do projeto — specs, tickets, revisões, retrospectivas. Quando um termo for usado fora da definição operacional, sinalizar explicitamente.
2. **Para agentes de IA:** este glossário deve ser incluído no contexto de prompt ou no system prompt de qualquer agente que opere sobre o domínio NovaTech. Termos marcados como ambíguos são candidatos a regras de detecção no pipeline (ex.: se o usuário menciona "Gold", o agente deve mapear para "tier Ouro" ou solicitar esclarecimento).
3. **Manutenção:** o glossário é atualizado junto com os bounded contexts. Novos termos entram quando identificados em feedbacks, sessões de revisão ou atualizações documentais.
